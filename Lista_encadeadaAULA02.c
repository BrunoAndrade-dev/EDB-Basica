...
// INSERINDO ELEMENTOS NO IN�CIO DA LISTA
...
// Implementando a fun��o LISTAR de forma recursiva
...
// Fun��o Busca de forma recursiva
Lista*buscaR(Lista *list , int v){
    if (list ==NULL) {
        return NULL ; // Alterado para retornar NULL em caso de n�o encontrado
    }
    if (list ->info ==v){
        return list ;
    }
...
Lista *retirar (Lista *list , int v) {
    ...
    if (p==NULL){
        return list; // N�o encontrou o elemento
    }

    // O n� p � o n� a ser removido
    if (ant == NULL){
        // Remover o primeiro n�
        list = p->next;
    } else {
        // Remover n� do meio ou fim
        ant ->next = p-> next;
    }
...
Lista*reverter(Lista*list){
    ...
    // O novo in�cio � 'anterior'
    return anterior;
}

// Fun��o remover n�s duplicados (assume que a lista n�o est� necessariamente ordenada)
    ...
    while (atual != NULL) {
        ...
        while (corrente->next != NULL) {
            if (atual->info == corrente->next->info) {
                // Encontrou duplicata: o n� a ser removido � corrente->next
                Lista *guardarduplicada = corrente->next;
                corrente->next = guardarduplicada->next;
                free(guardarduplicada);
            } else {
                // N�o � duplicata, avan�a o ponteiro corrente
                corrente = corrente->next;
            }
        }
        // Avan�a para o pr�ximo n� para verificar novas duplicatas
        atual = atual->next;
    }
    return topo; // Retorna o topo da lista
}

// Fun��o para Intercalar duas listas
...
Lista *intercalar (Lista *l1, Lista *l2) {
    ...
    Lista *aux1 = l1;
    Lista *aux2 = l2;
    Lista *ultimo = NULL; // Ponteiro para o �ltimo n� da l3

    while (aux1 != NULL || aux2 != NULL) {
        // Se houver elemento em l1, insere-o na l3
        if (aux1 != NULL) {
            // Cria novo n� com o valor de aux1->info
            Lista *novo = (Lista*)malloc(sizeof(Lista));
            if (novo == NULL) {
                printf("Erro de alocacao de memoria!\n");
                // Em um cen�rio real, voc� faria uma limpeza ou tratamento de erro mais robusto.
                exit(1);
            }
            novo->info = aux1->info;
            novo->next = NULL;
            ...
            if (l3 == NULL) {
                l3 = novo; // Primeiro n� da nova lista
            } else {
                ultimo->next = novo; // Anexa ao final da nova lista
            }
            ultimo = novo; // Atualiza o �ltimo n�

            aux1 = aux1->next; // Avan�a l1
        }

        // Se houver elemento em l2, insere-o na l3
        if (aux2 != NULL) {
            // Cria novo n� com o valor de aux2->info
            Lista *novo = (Lista*)malloc(sizeof(Lista));
            if (novo == NULL) {
                printf("Erro de alocacao de memoria!\n");
                exit(1);
            ...
            if (l3 == NULL) {
                l3 = novo; // Este caso deve ser raro se l1 n�o for nula, mas � seguro
            } else {
                ultimo->next = novo; // Anexa ao final da nova lista
            }
            ultimo = novo; // Atualiza o �ltimo n�

            aux2 = aux2->next; // Avan�a l2
        }
    }
 ...
 */
 // Fun��o verificar se duas listas est�o iguais
 int isIgual (Lista *l1 , Lista *l2){
    ...
    l2 = l2-> next ;
    if (l2 ==NULL && l1 == NULL) {
        return 1 ; // VERDADEIRO,OU SEJA, S�O IGUAIS
    }
    else{
        return 0 ; // FALSE,OU SEJA, N S�O IGUAIS
    }
 }
// Liberar a lista
Lista* liberar(Lista *list){
    ...
    return NULL; // Retorna NULL para indicar que a lista est� vazia
}

// Main
int main(){
    // Teste original (adaptado para os ajustes e corre��o de bugs na fun��o 'removerNO' e 'buscaR')
    ...
    mylista = removerNO(mylista); // A fun��o original tinha um bug no retorno e no loop
    printf("\nA nova lista (sem duplicatas) �: ");
    listar2(mylista);
    ...
    mylista = novacabeca; // mylista agora � a lista invertida
